...
public:
	...
	ref const(std_string)						GetName() const					{	return m_sName;	}
	void										SetName(ref const string sName)		{	m_sName = sName;	}
	int											GetID() const						{	return m_ID;		}
	ref CWeaponSystemEx				GetWeaponSystem()	const	{	return m_rWeaponSystem;	}

	// rendering related
	IStatObj.IStatObj*						GetObject() const			{	return m_pObject;	}
	ICryCharInstance*		GetCharacter() const	{	return m_pCharacter;	}
	IStatObj.IStatObj*						GetMuzzleFlash() const{	return m_pMuzzleFlash;	}
	ref const(std_string)				GetBindBone() const		{	return m_sBindBone;	}
	void								SetBindBone(ref const std_string sBindBone)	{	m_sBindBone = sBindBone;	}
	bool								LoadMuzzleFlash(ref const std_string sGeometryName);
	...
	ref const(Vec3) GetAngles() const	{	return m_vAngles;	}
	ref const(Vec3) GetPos() const	{	return m_vPos;	}
...
private:
	...
	int									m_ID;							//!< class ID
	std_string							m_sScript;				//!< script file representing this weapon class
...
public:
	...
	int						Fire(ref const Vec3d origin, ref const Vec3d angles, CPlayer *pPlayer, ref WeaponInfo winfo, IPhysicalEntity *pIRedirected);

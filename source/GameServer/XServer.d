...
auto BROADCAST_SLOTID(uint id)
{
	return cast(BYTE) id << 16;
} // Specify a slot
auto BROADCAST_MSG(uint msg)
{
	return cast(BYTE) msg << 24;
} // Specify a message
auto GETBROADCAST_SLOTID(uint data)
{
	return cast(BYTE)(data >> 16) & 0xFF;
}

auto GETBROADCAST_MSG(uint data)
{
	return cast(BYTE)(data >> 24) & 0xFF;
}
...
public:
	...
	/*override*/
	bool GetServerInfoStatus(ref std_string szName, ref std_string szGameType, ref std_string szMap,
			ref std_string szVersion, bool* pbPassword,
			int* piPlayers, int* piMaxPlayers);
	/*override*/
	bool GetServerInfoRules(ref std_string szServerRules);
	/*override*/
	bool GetServerInfoPlayers(std_string[4]* vszStrings, ref int nStrings);
	/*override*/
	bool ProcessXMLInfoRequest(const char* sRequest, const char* sRespone,
			int nResponseMaxLength);
	...
	void BroadcastCommand(const char* sCmd, const ref Vec3 invPos,

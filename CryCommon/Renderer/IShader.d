...
mixin(GenFWD!("CMaterial"));
mixin(GenFWD!("STexPic"));
mixin(GenFWD!("ICryCharInstance"));
mixin(GenFWD!("IDeformableRenderMesh"));
mixin(GenFWD!("CPShader"));

//template	<class T> class list2;
extern(C) struct SShader;
extern(C) struct SShaderPass;
...
public:
  ...
  //}
  //COrthoNormalBasis(ref Vec3 vForward, ref Vec3 vUp, ref Vec3 vRight)
  //{
  //  m_vForward = vForward;
  //  m_vUp = vUp;
  //  m_vRight = vRight;
  ...
  Matrix44 matrixBasisToDestBasis(ref const COrthoNormalBasis dest) const
  {
    return (dest.matrixXYZToBasis() * matrixBasisToXYZ());
  }

  ref COrthoNormalBasis rotate(ref Vec3 axis, const float degrees)
  {
    Matrix33 m;
  ...
  Vec3 m_vForward = Vec3(1,0,0); //tangent
  Vec3 m_vUp = Vec3(0,0,1);      //binormal
  Vec3 m_vRight = Vec3(0,1,0);   //normal
};


//////////////////////////////////////////////////////////////////////////
// CCObject::m_ObjFlags: Flags used by shader pipeline
enum FOB_SORTPOLYS = 1;
enum FOB_SELECTED  = 2;
enum FOB_NEAREST   = 4;
enum FOB_PORTAL    = 8;

enum FOB_CUBE_POSX = 0x10;
enum FOB_CUBE_NEGX = 0x20;
enum FOB_CUBE_POSY = 0x40;
enum FOB_CUBE_NEGY = 0x80;
enum FOB_CUBE_POSZ = 0x100;
enum FOB_CUBE_NEGZ = 0x200;
enum FOB_TEXTURE   = 0x400;
enum FOB_CUBE_MASK = 0x7f0;
enum FOB_CUBE_SHIFT = 4;
enum FOB_IGNOREMATERIALAMBIENT = 0x800;

enum FOB_DRSUN         = 0x1000;
enum FOB_REFRACTED     = 0x2000;

enum FOB_ZPASS         = 0x4000;
enum FOB_LIGHTPASS     = 0x8000;
enum FOB_FOGPASS       = 0x10000;
enum FOB_RENDER_INTO_SHADOWMAP = 0x20000;
enum FOB_SPECULAR      = 0x40000;
enum FOB_IGNOREREPOINTER = 0x80000;
enum FOB_ENVLIGHTING   = 0x100000;
enum FOB_BENDED        = 0x200000;
enum FOB_NOBUMP        = 0x400000;
enum FOB_HASALPHA      = 0x800000;
enum FOB_HOTAMBIENT    = 0x1000000;
enum FOB_MASKCONDITIONS  = (FOB_BENDED | FOB_HASALPHA | FOB_SPECULAR | FOB_NOBUMP | FOB_INSHADOW | FOB_HEATVISION | FOB_HOTAMBIENT | FOB_ENVLIGHTING);
enum FOB_REMOVED       = 0x2000000;
enum FOB_HEATVISION    = 0x4000000;
enum FOB_INSHADOW      = 0x8000000;

enum FOB_TRANS_ROTATE     = 0x10000000;
enum FOB_TRANS_SCALE      = 0x20000000;
enum FOB_TRANS_TRANSLATE  = 0x40000000;
enum FOB_TRANS_MASK  =(FOB_TRANS_ROTATE | FOB_TRANS_SCALE | FOB_TRANS_TRANSLATE);

enum FOB_NOSCISSOR     = 0x80000000;
...
// Size of CCObject currently is 256 ubytes. If you add new members better to ask me before or just
// make sure CCObject is cache line aligned (64 ubytes)

//mixin(GenFWD!("CCObject"));
extern(C) struct CCObject;
/*
version(FIXME_VERSION)
{
class CCObject
{
public:
  ...
  ref Matrix44 GetMatrix()
  {
    return m_Matrix;
  }

  ref Matrix44 GetInvMatrix();
  ref Matrix44 GetVPMatrix();

  void SetScissor();
  void SetAlphaState(CPShader *pPS, int nCurState);

  void AddWaves(SWaveForm2 **wf);
...
//#undef new
//#endif
	// Sergiy: it's enough to allocate 16 ubytes more, even on 64-bit machine
	// - and we need to store only the offset, not the actual pointer
  void* operator new( size_t Size )
  {
    ubyte *ptr = cast(ubyte *)malloc(Size+16+4);
    memset(ptr, 0, Size+16+4);
    ubyte *bPtrRes = (ubyte *)((INT_PTR)(ptr+4+16) & ~0xf);
    ((ubyte**)bPtrRes)[-1] = ptr;
  ...
  void* operator new[](size_t Size)
  {
    ubyte *ptr = cast(ubyte *)malloc(Size+16+2*sizeof(INT_PTR));
    memset(ptr, 0, Size+16+2*sizeof(INT_PTR));
    ubyte *bPtrRes = cast(ubyte *)((INT_PTR)(ptr+16+2*sizeof(INT_PTR)) & ~0xf);
    ((ubyte**)bPtrRes)[-2] = ptr;
  ...
  {
    ubyte *bActualPtr = ((ubyte **)Ptr)[-1];
		assert (bActualPtr <= (ubyte*)Ptr && (ubyte*)Ptr-bActualPtr < 20);
...
//////////////////////////////////////////////////////////////////////////
// Materials
enum LMF_IGNORELIGHTS = 1;
enum LMF_BUMPMATERIAL = 2;
enum LMF_NOAMBIENT    = 4;
enum LMF_COLMAT_AMB   = 8;
enum LMF_POLYOFFSET   = 0x10;
enum LMF_NOALPHA      = 0x20;
enum LMF_IGNOREPROJLIGHTS = 0x40;
enum LMF_NOBUMP       = 0x80;
enum LMF_DISABLE      = 0x100;
enum LMF_NOSPECULAR   = 0x200;
enum LMF_NOADDSPECULAR = 0x400;
enum LMF_HASPSHADER   = 0x800;
enum LMF_DIVIDEAMB4   = 0x1000;
enum LMF_DIVIDEAMB2   = 0x2000;
enum LMF_DIVIDEDIFF4  = 0x4000;
enum LMF_DIVIDEDIFF2  = 0x8000;

enum LMF_LIGHT0       = 0x10000;
enum LMF_LIGHT1       = 0x20000;
enum LMF_LIGHT2       = 0x30000;
enum LMF_LIGHT3       = 0x40000;
enum LMF_LIGHT4       = 0x50000;
enum LMF_LIGHT5       = 0x60000;
enum LMF_LIGHT6       = 0x70000;
enum LMF_LIGHT7       = 0x80000;

enum LMF_LIGHT_MASK   = 0xf0000;

enum LMF_1SAMPLES     = 0x100000;
enum LMF_2SAMPLES     = 0x200000;
enum LMF_3SAMPLES     = 0x400000;
enum LMF_4SAMPLES     = 0x800000;
enum LMF_SAMPLES      = 0xf00000;

enum LMF_LIGHT_SHIFT  = 16;

enum LMF_ONLYMATERIALAMBIENT = 0x1000000;
enum LMF_USEOCCLUSIONMAP     = 0x2000000;
enum LMF_HASAMBIENT          = 0x4000000;
enum LMF_HASDOT3LM           = 0x8000000;
...
struct SSideMaterial
{
  ...
  //FIXME:
  /*
  friend bool operator ==(const SSideMaterial &m1, const SSideMaterial &m2)
  {
...
struct SLightMaterial
{
  ...
  static TArray.TArray!(SLightMaterial *) known_materials;
};
...
enum DEF_TEXARG0 = (EColorArg.eCA_Texture|(EColorArg.eCA_Diffuse<<3));
enum DEF_TEXARG1 = (EColorArg.eCA_Texture|(EColorArg.eCA_Previous<<3));

//////////////////////////////////////////////////////////////////////////
// Animating Texture sequence definition
struct STexAnim
{
  TArray.TArray!(STexPic *) m_TexPics;
  int m_Rand;
  int m_NumAnimTexs;
  bool m_bLoop;
  float m_Time;
  ...
  int Size()
  {
    ...
    nSize += m_TexPics.GetSize() * (&STexPic).sizeof;
    return nSize;
  }
...
abstract class SGenTC     
{
  ...
  SGenTC *mfCopy();
  bool mfSet(bool bEnable);
  void mfCompile(char *params, SShader *ef);
  int Size();
};
...
enum FFILT_POINT  = 1;
enum FFILT_LINEAR = 2;
enum FFILT_ANISOTROPIC = 3;

enum FTU_OPAQUE = 1;
enum FTU_CLAMP  = 2;
enum FTU_NOMIPS = 4;
enum FTU_NOBUMP = 8;
enum FTU_PROJECTED       = 0x10;
enum FTU_FILTERBILINEAR  = 0x20;
enum FTU_FILTERTRILINEAR = 0x40;
enum FTU_FILTERNEAREST   = 0x80;
enum FTU_FILTERLINEAR    = 0x100;
enum FTU_BUMPPLANTS      = 0x1000;
enum FTU_NOSCALE         = 0x2000;
...
template CASE_TEXMOD(string var_name)
{
    const char[] CASE_TEXMOD = "
	if(!strcmp(" ~ var_name ~ ",szParamName))
	{
		" ~ var_name ~ " = fValue;
		return true;
	}
...
template CASE_TEXMODANGLE(string var_name)
{
    const char[] CASE_TEXMODANGLE = "
	if(!strcmp(" ~ var_name ~ ",szParamName))
	{
...
template CASE_TEXMODubyte(string var_name)
{
    const char[] CASE_TEXMODubyte = "
	if(!strcmp(" ~ var_name ~ ",szParamName))
	{
		" ~ var_name ~ " = cast(ubyte)fValue;
		return true;
	}
    ";
}

template CASE_TEXMODBOOL(string var_name)
{
    const char[] CASE_TEXMODBOOL = "
	if(!strcmp(" ~ var_name ~ ",szParamName))
	{
		" ~ var_name ~ " =(fValue==1.f);
		return true;
...
struct SEfTexModificator
{
	...
	{
		mixin(CASE_TEXMODubyte!(m_eTGType));
		mixin(CASE_TEXMODubyte!(m_eRotType));
		mixin(CASE_TEXMODubyte!(m_eUMoveType));
		mixin(CASE_TEXMODubyte!(m_eVMoveType));
		mixin(CASE_TEXMODBOOL!(m_bTexGenProjected));

		mixin(CASE_TEXMOD!(m_Tiling[0]));
		mixin(CASE_TEXMOD!(m_Tiling[1]));
		mixin(CASE_TEXMOD!(m_Tiling[2]));
		mixin(CASE_TEXMOD!(m_Offs[0]));
		mixin(CASE_TEXMOD!(m_Offs[1]));
		mixin(CASE_TEXMOD!(m_Offs[2]));

		mixin(CASE_TEXMODANGLE!(m_Rot[0]));
		mixin(CASE_TEXMODANGLE!(m_Rot[1]));
		mixin(CASE_TEXMODANGLE!(m_Rot[2]));
		mixin(CASE_TEXMODANGLE!(m_RotOscRate[0]));
		mixin(CASE_TEXMODANGLE!(m_RotOscRate[1]));
		mixin(CASE_TEXMODANGLE!(m_RotOscRate[2]));
		mixin(CASE_TEXMODANGLE!(m_RotOscAmplitude[0]));
		mixin(CASE_TEXMODANGLE!(m_RotOscAmplitude[1]));
		mixin(CASE_TEXMODANGLE!(m_RotOscAmplitude[2]));
		mixin(CASE_TEXMODANGLE!(m_RotOscPhase[0]));
		mixin(CASE_TEXMODANGLE!(m_RotOscPhase[1]));
		mixin(CASE_TEXMODANGLE!(m_RotOscPhase[2]));
		mixin(CASE_TEXMOD!(m_RotOscCenter[0]));
		mixin(CASE_TEXMOD!(m_RotOscCenter[1]));
		mixin(CASE_TEXMOD!(m_RotOscCenter[2]));

		mixin(CASE_TEXMOD!(m_UOscRate));
		mixin(CASE_TEXMOD!(m_VOscRate));
		mixin(CASE_TEXMOD!(m_UOscAmplitude));
		mixin(CASE_TEXMOD!(m_VOscAmplitude));
		mixin(CASE_TEXMOD!(m_UOscPhase));
		mixin(CASE_TEXMOD!(m_VOscPhase));
  ...
  ushort[3] m_Rot;
  ushort[3] m_RotOscRate;
  ushort[3] m_RotOscAmplitude;
  ushort[3] m_RotOscPhase;
  float[3] m_RotOscCenter;
...
//FIXME:
//  friend bool operator != (const SEfTexModificator &m1, const SEfTexModificator &m2)
//  {
...
struct SEfResTexture
{
  ...
  //FIXME:
  //friend bool operator != (const SEfResTexture &m1, const SEfResTexture &m2)
  //{
...
enum EFTT_DIFFUSE           = 0;
enum EFTT_BUMP              = 1;
enum EFTT_BUMP_DIFFUSE      = 2;
enum EFTT_GLOSS             = 3;
enum EFTT_CUBEMAP           = 4;
enum EFTT_PHONG             = 5;
enum EFTT_BUMP_HEIGHT       = 6;
enum EFTT_ATTENUATION2D     = 7;
enum EFTT_SPECULAR          = 8;
enum EFTT_DETAIL_OVERLAY    = 9;
enum EFTT_REFLECTION        = 10;
enum EFTT_SUBSURFACE        = 11;
enum EFTT_ATTENUATION1D     = 12;
enum EFTT_OPACITY           = 13;
enum EFTT_LIGHTMAP          = 14;
enum EFTT_LIGHTMAP_HDR      = 15;
enum EFTT_LIGHTMAP_DIR      = 16;
enum EFTT_OCCLUSION         = 17;
enum EFTT_DECAL_OVERLAY     = 18;
enum EFTT_NORMALMAP         = 19;

enum EFTT_MAX               = 20;

class SBaseShaderResources
{
  int m_ResFlags;
  float m_Opacity = 1;
  float m_AlphaRef;
  SLightMaterial *m_LMaterial;
  TArray.TArray!SShaderParam m_ShaderParams;
  // in order to facilitate the memory allocation tracking, we're using here this class;
  // if you don't like it, please write a substitute for all string within the project and use them everywhere
...
//////////////////////////////////////////////////////////////////////////
class SRenderShaderResources : SBaseShaderResources
{
public:
  int m_Id;
  int m_nRefCounter;
  ...
  bool m_bNeedNormals;
  SEfResTexture[EFTT_MAX] *m_Textures;
...
class SInputShaderResources : SBaseShaderResources
{
public:
  SEfResTexture[EFTT_MAX] m_Textures;
  ...
  override int Size()
  {
    int nSize = super.Size() - sizeof(SEfResTexture) * EFTT_MAX;
...
struct SShaderGen
{
	TArray.TArray!(SShaderGenBit *) m_BitMask;
};

//////////////////////////////////////////////////////////////////////////
// Template types
enum EFT_DEFAULT      = 0;  // Default shader as it is
enum EFT_DECAL        = 1;  // Only decal without lights
enum EFT_INVLIGHT   = 3;  // Decal with HW lights (DLight should be added)
enum EFT_WHITESHADOW = 4;  // Depth map
enum EFT_WHITE        = 7;  // Just white texture with Z write
enum EFT_FROMMAX      = 14;  // Max
enum EFT_HEATVISION  = 19; // Heat vision template
enum EFT_DOF         = 20; // Depth of field template
enum EFT_USER_FIRST    = 30;   // First user defined template (each user's template must be registered)
...
//////////////////////////////////////////////////////////////////////////
enum EFSLIST_PREPROCESS_ID = 2;
enum EFSLIST_DISTSORT_ID   = 1;
enum EFSLIST_GENERAL_ID    = 0;
enum EFSLIST_STENCIL_ID    = 3;
enum EFSLIST_LAST_ID       = 4;
enum EFSLIST_UNSORTED_ID   = 5;
enum EFSLIST_MASK    = (15<<28);

enum EFSLIST_PREPROCESS = (EFSLIST_PREPROCESS_ID << 28); // Rendering 1st  (preprocess items);
enum EFSLIST_STENCIL    = (EFSLIST_STENCIL_ID << 28);    // Rendering 2nd (opaque surfaces with stencil shadows supporting);
enum EFSLIST_LAST       = (EFSLIST_LAST_ID << 28);       // Rendering last (6th) (sorted list of all fog passes, screen shaders, ...);
enum EFSLIST_GENERAL    = (EFSLIST_GENERAL_ID << 28);    // Rendering 4th  (default sorted list for most render items);
enum EFSLIST_DISTSORT   = (EFSLIST_DISTSORT_ID << 28);   // Rendering 5th (blended sorted by distance render items);
enum EFSLIST_UNSORTED   = (EFSLIST_UNSORTED_ID << 28);  // Rendering 3rd (unsorted shadow passes on terrain);

//////////////////////////////////////////////////////////////////////////
struct SEfTemplates
{
  TArray.TArray!(SShader *) m_TemplShaders;  // All templates (Shaders)
  int m_nPreferred = -1;
  int m_nMaskAuto;
  SShader *m_Preferred;
...
enum SPRID_CORONA         = 0; 
enum FSPR_CORONA          = (1<<SPRID_CORONA);
enum SPRID_NIGHTVIS       = 1;
enum FSPR_NIGHTVIS        = (1<<SPRID_NIGHTVIS);
enum SPRID_DOFMAP         = 2;
enum FSPR_DOFMAP          = (1<<SPRID_DOFMAP);
enum SPRID_PORTAL         = 3;
enum FSPR_PORTAL          = (1<<SPRID_PORTAL);
enum SPRID_SCANCM         = 4;
enum FSPR_SCANCM          = (1<<SPRID_SCANCM);
enum SPRID_SCANSCR        = 5;
enum FSPR_SCANSCR         = (1<<SPRID_SCANSCR);
enum SPRID_SCANTEXWATER   = 6;
enum FSPR_SCANTEXWATER    = (1<<SPRID_SCANTEXWATER);
enum SPRID_SCANTEX        = 7;
enum FSPR_SCANTEX         = (1<<SPRID_SCANTEX);
enum SPRID_SCANLCM        = 8;
enum FSPR_SCANLCM         = (1<<SPRID_SCANLCM);
enum SPRID_CUSTOMCM       = 9;
enum FSPR_CUSTOMCM        = (1<<SPRID_CUSTOMCM);
enum SPRID_CUSTOMTEXTURE  = 10;
enum FSPR_CUSTOMTEXTURE   = (1<<SPRID_CUSTOMTEXTURE);
enum SPRID_FLASHBANG      = 11;
enum FSPR_FLASHBANG       = (1<<SPRID_FLASHBANG);

enum SPRID_SHADOWMAPGEN   = 12;
enum FSPR_SHADOWMAPGEN    = (1<<SPRID_SHADOWMAPGEN);
enum SPRID_RAINOVERLAY    = 13;
enum FSPR_RAINOVERLAY     = (1<<SPRID_RAINOVERLAY);
enum SPRID_REFRACTED      = 14;
enum FSPR_REFRACTED       = (1<<SPRID_REFRACTED);

  // tiago: added
enum SPRID_HEATVIS        = 15;
enum FSPR_HEATVIS         = (1<<SPRID_HEATVIS);
enum SPRID_SCREENTEXMAP   = 16;
enum FSPR_SCREENTEXMAP    = (1<<SPRID_SCREENTEXMAP);

enum FSPR_MAX             = 0x40000;


// SShader::m_Flags
// Different useful flags
enum EF_NEEDTANGENTS  = 1;       // Shader needs tangent vectors array
enum EF_BUMPAUTOPROC  = 2;
enum EF_BUMPAUTOPIC   = 4;
enum EF_HASVSHADER    = 8;
enum EF_HASCULL       = 0x10;
enum EF_NOBREAKABLELIGHT = 0x20;
enum EF_OVERLAY       = 0x40;
enum EF_ORIENT        = 0x80;
enum EF_MATERIAL      = 0x100;
enum EF_TEMPLNAMES    = 0x200;
enum EF_SKY_HDR       = 0x400;
enum EF_LIGHTSTYLE    = 0x800;
enum EF_SUNFLARES     = 0x2000;
enum EF_NEEDNORMALS   = 0x4000;  // Need normals operations
enum EF_OFFSETBUMP    = 0x8000;
enum EF_NOTFOUND      = 0x10000;
enum EF_DEFAULT       = 0x20000;
enum EF_SKY           = 0x40000;
enum EF_USELIGHTS     = 0x80000;
enum EF_ALLOW3DC      = 0x100000;
enum EF_FOGSHADER     = 0x200000;
enum EF_USEPROJLIGHTS = 0x400000;
enum EF_POLYGONOFFSET = 0x800000;
enum EF_NOMIPMAPS     = 0x1000000;
enum EF_COMPILEDLAYERS = 0x2000000;
enum EF_USEMTLAYERS   = 0x4000000;
enum EF_DIFFUSELIGHT  = 0x8000000;
enum EF_CLIENTEFFECT  = 0x10000000;
enum EF_SYSTEM        = 0x20000000;
enum EF_HASDIFFUSEMAP = 0x40000000;
enum EF_NOSPOTS       = 0x80000000;

// SShader::Flags2
// Additional Different useful flags
enum EF2_ALLOW_FENCE    = 0x1;
enum EF2_PREPR_OUTSPACE = 0x2;
enum EF2_NOCASTSHADOWS  = 0x4;
enum EF2_FOGOVERLAY1    = 0x8;
enum EF2_FOGOVERLAY2    = 0x10;
enum EF2_FOGOVERLAY     = 0x18;
enum EF2_TESSSIZE       = 0x20;
enum EF2_HASOPAQUE      = 0x40;
enum EF2_CUSTOMANIMTEX  = 0x80;
enum EF2_DONTSORTBYDIST = 0x100;
enum EF2_HASSUNFLARE    = 0x200;
enum EF2_OPAQUE         = 0x400;
enum EF2_TEMPLATE       = 0x800;
enum EF2_IGNORERESOURCESTATES  = 0x1000;
enum EF2_USELIGHTMATERIAL  = 0x2000;
enum EF2_REDEPEND       = 0x4000;
enum EF2_DEFAULTVERTEXFORMAT = 0x8000;

// SShader::Flags3
// Additional Different useful flags
enum EF3_NODRAW            = 1;
enum EF3_NEEDSYSBUF        = 2;
enum EF3_HASLM             = 4;
enum EF3_HASRGBGEN         = 8;
enum EF3_HASALPHAGEN       = 0x10;
enum EF3_CLIPPLANE_BACK    = 0x20;
enum EF3_CLIPPLANE_FRONT   = 0x40;
enum EF3_CLIPPLANE_WATER_FRONT = 0x80;
enum EF3_CLIPPLANE_WATER_BACK  = 0x100;
enum EF3_CLIPPLANE =         (EF3_CLIPPLANE_BACK | EF3_CLIPPLANE_FRONT | EF3_CLIPPLANE_WATER_BACK | EF3_CLIPPLANE_WATER_FRONT);
enum EF3_SCREENTEXTURE       = 0x200;
enum EF3_IGNOREDIRECTIONALLIGHT  = 0x400;
enum EF3_HASVCOLORS        = 0x1000;
enum EF3_HASALPHATEST      = 0x2000;
enum EF3_HASALPHABLEND     = 0x4000;
enum EF3_USEPARENTSORT     = 0x8000;
enum EF3_USEPARENTCULL     = 0x10000;
enum EF3_REFLECTION        = 0x20000;
enum EF3_REBUILD           = 0x40000;
enum EF3_NODETAIL          = 0x80000;
enum EF3_DEPTHWRITE        = 0x100000;
enum EF3_NOTEMPLATE        = 0x200000;
enum EF3_HASRCOMBINER      = 0x400000;
enum EF3_HASPSHADER        = 0x800000;
enum EF3_HASAMBPASSES      = 0x1000000;
enum EF3_TESSSIZE          = 0x2000000;
enum EF3_SHAREVERTS        = 0x4000000;

enum EF3_PREPARELV      = 0x10000000;
enum EF3_PREPAREHAV     = 0x20000000;
enum EF3_PREPARELAS0    = 0x40000000;
enum EF3_PREPARELAS1    = 0x80000000;
enum EF3_PREPARE_MASK =   (EF3_PREPARELV | EF3_PREPAREHAV | EF3_PREPARELAS0 | EF3_PREPARELAS1);
...
public:
  ...
  const char *GetName();
	...
	int GetFlags3();
  int GetRenderFlags();
  void SetRenderFlags(int nFlags);
  int GetLFlags();
  int GetCull();
  uint GetPreprocessFlags();
	void SetFlags3(int Flags);
  bool Reload(int nFlags);
  TArray.TArray!(CRendElement *) *GetREs ();
  bool AddTemplate(SRenderShaderResources *Res, ref int TemplId, const char *Name=null, bool bSetPreferred=false, uint64 nMaskGen=0);
  void RemoveTemplate(int TemplId);
  IShader *GetTemplate(int num);
  SEfTemplates *GetTemplates();
  ref TArray.TArray!SShaderParam GetPublicParams();
  int GetTexId ();
  ITexPic *GetBaseTexture(int *nPass, int *nTU);
  uint GetUsedTextureTypes();
  int GetVertexFormat();
  int Size(int Flags);
	uint64 GetGenerationMask();
	SShaderGen* GetGenerationParams();
};
...
//////////////////////////////////////////////////////////////////////////
enum MIF_POLYBUMP      = 1;
enum MIF_INVPOLYBUMP   = 2;
enum MIF_PHYSIC        = 4;
enum MIF_NOCASTSHADOWS = 8;
enum MIF_COLLIDABLE	 = 16;
enum MIF_INVALID			 = 32; // Set when material marked as invalid
enum MIF_WASUSED			 = 64; // Set when material assigned to some object
enum MIF_CHILD				= 128; // Set if material is parent material
...
interface IMatInfo
{
  ...
  int GetNumRefs();
	...
	// shader item
	void SetShaderItem(ref SShaderItem  _ShaderItem);
	const ref SShaderItem  GetShaderItem();

	//! Used by custom material to override shader item.
	bool OverrideShaderItem( int subMtlId,ref SShaderItem si );
	
	// shader params
	void SetShaderParams(TArray.TArray!SShaderParam * _pShaderParams);
	const TArray.TArray!SShaderParam * GetShaderParams();
...
public:
  ...
  int GetNumRefs() { return m_nRefCount; };

  char[32] sScriptMaterial;
	char[64] sMaterialName;

	//! Number of references to this material.
	int m_nRefCount;
  int m_Id = -1;
	int m_nCGFMaterialID = -1;
	//! Material flags.
	//! @see EMatInfoFlags
  int m_Flags;
  int  nGamePhysMatId;
  ...
  float fAlpha = -1;
	...
	MAT_ENTITY * pMatEnt; // used by resource compiler
	TArray.TArray!SShaderParam *pShaderParams;

	//! Array of Sub materials.
	alias TArray.TArray!(IMatInfo*) SubMtls;
	SubMtls *pSubMtls;
  ...
  //};
	...
	const char* GetName() { return sMaterialName; };

	void SetFlags( int flags ) { m_Flags = flags; };
	int GetFlags() const { return m_Flags; };

	// shader item
	void SetShaderItem(ref SShaderItem  _ShaderItem) { shaderItem = _ShaderItem; }
	ref const SShaderItem  GetShaderItem() { return shaderItem; };
	
	// shader params
	void SetShaderParams(TArray.TArray!SShaderParam * _pShaderParams) { pShaderParams = _pShaderParams; }
	const TArray.TArray!SShaderParam * GetShaderParams() { return pShaderParams; }
	...
	bool OverrideShaderItem( int subMtlId,ref SShaderItem si )
	{
		...
		if (pSubMtls)
		{
			if (subMtlId-1 < cast(int)pSubMtls.size())
			{
...
// DLights
enum DLF_DETAIL					= 1;
enum DLF_DIRECTIONAL			= 2;
enum DLF_DYNAMIC					= 4;							//dynamic lights
enum DLF_ACTIVE					= 8;							//light is active / inactive
enum DLF_CASTSHADOW_MAPS = 0x10;					//light casting shadows
enum DLF_POINT						= 0x20;
enum DLF_PROJECT					= 0x40;
enum DLF_CASTSHADOW_VOLUME			= 0x80;		//light casting shadows
enum DLF_POSITIONCHANGED = 0x100;
enum DLF_NOATTENUATION   = 0x200;
enum DLF_UPDATED					= 0x400;
enum DLF_INWORLDSPACE  	= 0x800;
enum DLF_TEMP   					= 0x1000;
enum DLF_STATIC_ADDED		= 0x2000;				//this static light has been already added to the list
enum DLF_HASAMBIENT			= 0x4000;
enum DLF_HEATSOURCE			= 0x8000;
enum DLF_LIGHTSOURCE			= 0x10000;
enum DLF_FAKE      			= 0x20000;				//actually it's not LS, just render elements (Flares, beams, ...)
enum DLF_SUN      			  = 0x40000;				//only sun may use this flag
enum DLF_COPY     			  = 0x80000;
enum DLF_LOCAL    			  = 0x100000;
enum DLF_LM      			  = 0x200000;
enum DLF_THIS_AREA_ONLY  = 0x400000;			// affects only current area/sector
enum DLF_AMBIENT_LIGHT		= 0x800000;			// only used to add better ambient lighting to polybump characters
enum DLF_IGNORE_OWNER		= 0x1000000;			// do not affect light owner object
enum DLF_IGNORE_TERRAIN	= 0x2000000;			// do not affect heightmap
enum DLF_ONLY_FOR_HIGHSPEC	= 0x4000000;		//!< This light is active as dynamic light only for high spec machines.
enum DLF_SPECULAR_ONLY_FOR_HIGHSPEC	= 0x8000000; //!< This light have specular component enabled only for high spec machines.
enum DLF_LMDOT3          = 0x10000000;
enum DLF_FAKE_RADIOSITY  = 0x20000000;
enum DLF_LMOCCL					= 0x40000000;

enum DLF_LIGHTTYPE_MASK = (DLF_DIRECTIONAL | DLF_POINT | DLF_PROJECT);
...
public:
  ...
  COrthoNormalBasis								m_Orientation;
	...
	CCObject[4][4] *											m_pObject;								//!< Object for light coronas and light flares
  ...
  uint														m_Flags = DLF_LIGHTSOURCE;									//!< flags from above (prefix DLF_)
  ...
  char[8]														m_sDebugName;					//!< name of light creator (for debuging, pointer can't be used since entity may be deleted)

  ShadowMapLightSourceInstance *	m_pShadowMapLightSource;	//!<

  CLeafBuffer[8] *										m_arrLightLeafBuffers; //!< array of leafbuffers used for heightmap lighting pass

module LeafBuffer;

import irenderer;
import ishader;
import Renderer.VertexFormats;
import Renderer.VertexBufferSource;
import CryHeaders;

extern(C++):

//////////////////////////////////////////////////////////////////////////
//! structure for tangent basises storing
struct TangData
{
	Vec3 tangent;
	Vec3 binormal;			
  Vec3 tnormal;			
};

//////////////////////////////////////////////////////////////////////////
struct CLeafBuffer
{
	//! constructor
	//! /param szSource this pointer is stored - make sure the memory stays
	this( const char *szSource );

	//! destructor
  ~this();
	
  static CLeafBuffer			m_Root;

  CLeafBuffer *						m_Next;						//!<
  CLeafBuffer *						m_Prev;						//!<
  char *									m_sSource;				//!< pointer to the give name in the constructor call

  void Unlink()
  {
    if (!m_Next || !m_Prev)
      return;
    m_Next.m_Prev = m_Prev;
    m_Prev.m_Next = m_Next;
    m_Next = m_Prev = null;
  }
  void Link( CLeafBuffer* Before )
  {
    if (m_Next || m_Prev)
      return;
    m_Next = Before.m_Next;
    Before.m_Next.m_Prev = this;
    Before.m_Next = this;
    m_Prev = Before;
  }

  static CLeafBuffer			m_RootGlobal;							//!<
  CLeafBuffer *						m_NextGlobal;							//!<
  CLeafBuffer *						m_PrevGlobal;							//!<

  void UnlinkGlobal()
  {
    if (!m_NextGlobal || !m_PrevGlobal)
      return;
    m_NextGlobal.m_PrevGlobal = m_PrevGlobal;
    m_PrevGlobal.m_NextGlobal = m_NextGlobal;
    m_NextGlobal = m_PrevGlobal = null;
  }
  void LinkGlobal( CLeafBuffer* Before )
  {
    if (m_NextGlobal || m_PrevGlobal)
      return;
    m_NextGlobal = Before.m_NextGlobal;
    Before.m_NextGlobal.m_PrevGlobal = this;
    Before.m_NextGlobal = this;
    m_PrevGlobal = Before;
  }

  Vec3 *	      			m_TempNormals;								//!<
  SMRendTexVert *			m_TempTexCoords;							//!<
  UCol *							m_TempColors;									//!<
  UCol *							m_TempSecColors;							//!<

  void *							m_pCustomData;								//!< userdata, useful for PrepareBufferCallback, currently this is used only for CDetailGrass

  alias bool function (CLeafBuffer *Data, bool bNeedTangents) PrepareBufferCallback;

  void SetVertexContainer(CLeafBuffer *pBuf)
  {
    m_pVertexContainer = pBuf;
  }
  CLeafBuffer *GetVertexContainer()
  {
    if (m_pVertexContainer)
      return m_pVertexContainer;
    return this;
  }

  ubyte *GetNormalPtr(ref int Stride, int Id=0, bool bSys=true)
  {
    CLeafBuffer *lb = GetVertexContainer();
    ubyte *pData;
    SBufInfoTable * pOffs;
    if (bSys)
    {
      pData = cast(ubyte *)lb.m_pSecVertBuffer.m_VS[VSF_GENERAL].m_VData;
      pOffs = &gBufInfoTable[lb.m_pSecVertBuffer.m_vertexformat];
      Stride = m_VertexSize[lb.m_pSecVertBuffer.m_vertexformat];
    }
    else
    {
      pData = cast(ubyte *)lb.m_pVertexBuffer.m_VS[VSF_GENERAL].m_VData;
      pOffs = &gBufInfoTable[lb.m_pVertexBuffer.m_vertexformat];
      Stride = m_VertexSize[lb.m_pVertexBuffer.m_vertexformat];
    }
    if (pOffs.OffsNormal)
    {
      return &pData[Id*Stride+pOffs.OffsNormal];
    }

    Stride = sizeof(Vec3);
    return cast(ubyte*)&lb.m_TempNormals[Id];
  }

  ubyte *GetPosPtr(ref int Stride, int Id=0, bool bSys=true)
  {
    CLeafBuffer *lb = GetVertexContainer();
    ubyte *pData;
    if (bSys)
    {
      pData = cast(ubyte *)lb.m_pSecVertBuffer.m_VS[VSF_GENERAL].m_VData;
      Stride = m_VertexSize[lb.m_pSecVertBuffer.m_vertexformat];
    }
    else
    {
      pData = cast(ubyte *)lb.m_pVertexBuffer.m_VS[VSF_GENERAL].m_VData;
      Stride = m_VertexSize[lb.m_pVertexBuffer.m_vertexformat];
    }
    return &pData[Id*Stride];
  }

  ubyte *GetBinormalPtr(ref int Stride, int Id=0, bool bSys=true)
  {
    CLeafBuffer *lb = GetVertexContainer();
    ubyte *pData;
    if (bSys)
    {
      pData = cast(ubyte *)lb.m_pSecVertBuffer.m_VS[VSF_TANGENTS].m_VData;
    }
    else
    {
      pData = cast(ubyte *)lb.m_pVertexBuffer.m_VS[VSF_TANGENTS].m_VData;
    }
    Stride = sizeof(SPipTangents);
    return &pData[Id*Stride+12];
  }
  ubyte *GetTangentPtr(ref int Stride, int Id=0, bool bSys=true)
  {
    CLeafBuffer *lb = GetVertexContainer();
    ubyte *pData;
    if (bSys)
    {
      pData = cast(ubyte *)lb.m_pSecVertBuffer.m_VS[VSF_TANGENTS].m_VData;
    }
    else
    {
      pData = cast(ubyte *)lb.m_pVertexBuffer.m_VS[VSF_TANGENTS].m_VData;
    }
    Stride = sizeof(SPipTangents);
    return &pData[Id*Stride];
  }
  ubyte *GetTNormalPtr(ref int Stride, int Id=0, bool bSys=true)
  {
    CLeafBuffer *lb = GetVertexContainer();
    ubyte *pData;
    if (bSys)
    {
      pData = cast(ubyte *)lb.m_pSecVertBuffer.m_VS[VSF_TANGENTS].m_VData;
    }
    else
    {
      pData = cast(ubyte *)lb.m_pVertexBuffer.m_VS[VSF_TANGENTS].m_VData;
    }
    Stride = sizeof(SPipTangents);
    return &pData[Id*Stride+24];
  }

  ubyte *GetColorPtr(ref int Stride, int Id=0, bool bSys=true)
  {
    CLeafBuffer *lb = GetVertexContainer();
    SBufInfoTable * pOffs;
    ubyte *pData;
    if (bSys)
    {
      pData = cast(ubyte *)lb.m_pSecVertBuffer.m_VS[VSF_GENERAL].m_VData;
      pOffs = &gBufInfoTable[lb.m_pSecVertBuffer.m_vertexformat];
      Stride = m_VertexSize[lb.m_pSecVertBuffer.m_vertexformat];
    }
    else
    {
      pData = cast(ubyte *)lb.m_pVertexBuffer.m_VS[VSF_GENERAL].m_VData;
      pOffs = &gBufInfoTable[lb.m_pVertexBuffer.m_vertexformat];
      Stride = m_VertexSize[lb.m_pVertexBuffer.m_vertexformat];
    }
    if (pOffs.OffsColor)
    {
      return &pData[Id*Stride+pOffs.OffsColor];
    }

    Stride = sizeof(UCol);
    return cast(ubyte*)&lb.m_TempColors[Id];
  }

  ubyte *GetSecColorPtr(ref int Stride, int Id=0, bool bSys=true)
  {
    CLeafBuffer *lb = GetVertexContainer();
    SBufInfoTable * pOffs;
    ubyte *pData;
    if (bSys)
    {
      pData = cast(ubyte *)lb.m_pSecVertBuffer.m_VS[VSF_GENERAL].m_VData;
      pOffs = &gBufInfoTable[lb.m_pSecVertBuffer.m_vertexformat];
      Stride = m_VertexSize[lb.m_pSecVertBuffer.m_vertexformat];
    }
    else
    {
      pData = cast(ubyte *)lb.m_pVertexBuffer.m_VS[VSF_GENERAL].m_VData;
      pOffs = &gBufInfoTable[lb.m_pVertexBuffer.m_vertexformat];
      Stride = m_VertexSize[lb.m_pVertexBuffer.m_vertexformat];
    }
    if (pOffs.OffsSecColor)
    {
      return &pData[Id*Stride+pOffs.OffsSecColor];
    }
    return null;
  }

  ubyte *GetUVPtr(ref int Stride, int Id=0, bool bSys=true)
  {
    CLeafBuffer *lb = GetVertexContainer();
    SBufInfoTable * pOffs;
    ubyte *pData;
    if (bSys)
    {
      pData = cast(ubyte *)lb.m_pSecVertBuffer.m_VS[VSF_GENERAL].m_VData;
      pOffs = &gBufInfoTable[lb.m_pSecVertBuffer.m_vertexformat];
      Stride = m_VertexSize[m_pSecVertBuffer.m_vertexformat];
    }
    else
    {
      pData = cast(ubyte *)lb.m_pVertexBuffer.m_VS[VSF_GENERAL].m_VData;
      pOffs = &gBufInfoTable[lb.m_pVertexBuffer.m_vertexformat];
      Stride = m_VertexSize[m_pVertexBuffer.m_vertexformat];
    }
    if (pOffs.OffsTC)
    {
      return &pData[Id*Stride+pOffs.OffsTC];
    }

    Stride = SMRendTexVert.sizeof;
    return cast(ubyte*)&lb.m_TempTexCoords[Id];
  }
  
  CVertexBuffer *					m_pVertexBuffer;						//!< video memory

  CLeafBuffer *						m_pVertexContainer;					//!<

  //FIXME:!!! uncomment and convert
  //uint										m_bMaterialsWasCreatedInRenderer : 1;
  //uint										m_bOnlyVideoBuffer : 1;
  //uint										m_bDynamic : 1;

  uint										m_UpdateVBufferMask;				//!<
  uint                    m_UpdateFrame;
  uint                    m_SortFrame;								//!< to prevent unneccessary sorting during one frame
  int											m_SecVertCount;							//!< number of vertices in m_pSecVertBuffer
  CVertexBuffer *					m_pSecVertBuffer;						//!< system memory

  SVertexStream           m_Indices;
  TArray!ushort         m_SecIndices;
  int                     m_NumIndices;
  list2!ushort *         m_pIndicesPreStrip;

  uint *									m_arrVertStripMap;					//!<

  int						          m_nVertexFormat;
	int											m_nPrimetiveType;						//!< R_PRIMV_TRIANGLES, R_PRIMV_TRIANGLE_STRIP, R_PRIMV...

  list2!CMatInfo *				m_pMats;										//!<

	uint *									m_arrVtxMap;												//!< [Anton] mapping table leaf buffer vertex idx.original vertex idx
  float										m_fMinU, m_fMinV, m_fMaxU, m_fMaxV;	//!< only needed for fur rendering (room for improvement)

	//! /param StripType e.g. STRIPTYPE_NONE,STRIPTYPE_ONLYLISTS,STRIPTYPE_SINGLESTRIP,STRIPTYPE_MULTIPLESTRIPS,STRIPTYPE_DEFAULT (in IRenderer.h)
  void StripifyMesh( int inStripType );

	void CalcFaceNormals();

  void AddRE(CCObject * pObj, IShader * pEf, int nNumSort=0, IShader * pStateEff = null);

  void SaveTexCoords(ubyte *pD, SBufInfoTable *pOffs, int Size);
  void SaveColors(ubyte *pD, SBufInfoTable *pOffs, int Size);
  void SaveSecColors(ubyte *pD, SBufInfoTable *pOffs, int Size);
  void SaveNormals(ubyte *pD, SBufInfoTable *pOffs, int Size);

  void SortTris();

  bool ReCreateSystemBuffer(int VertFormat);
  void UpdateDynBufPtr(int VertFormat);
  bool CheckUpdate(int VertFormat, int Flags, bool bNeedAddNormals);
  bool CreateTangBuffer();
  void ReleaseShaders();
  void InvalidateVideoBuffer(int flags=-1) { m_UpdateVBufferMask |= flags; };
  void CopyTo(CLeafBuffer *pDst, bool bUseSysBuf);
  
  extern(C) struct CIndexedMesh;
  void CreateBuffer( CIndexedMesh * pTriData, bool bStripifyAndShareVerts, bool bRemoveNoDrawFaces = true, bool bKeepRemapTable = false);
	bool CreateBuffer( VertexBufferSource* pSource );

  int GetAllocatedubytes( bool bVideoMem );

  // compact buffer
  int FindInBuffer(ref struct_VERTEX_FORMAT_P3F_N_COL4UB_TEX2F opt, ref SPipTangents origBasis, uint nMatInfo, uint *uiInfo, struct_VERTEX_FORMAT_P3F_N_COL4UB_TEX2F* _vbuff, SPipTangents *_vbasis, int _vcount, list2!ushort * pHash, ref TArray!uint ShareNewInfo);
  void CompactBuffer(struct_VERTEX_FORMAT_P3F_N_COL4UB_TEX2F *_vbuff, SPipTangents *_tbuff, int * _vcount, TArray!ushort * pindices, bool[128] bShareVerts, uint *uiInfo);

  void Unload();
  void UpdateCustomLighting(float fBackSideLevel, Vec3 vStatObjAmbientColor, ref const Vec3  vLight, bool bCalcLighting);
  
  void AddRenderElements(CCObject * pObj=null, int DLightMask=0, int nTemplate=-1, int nFogVolumeID=0, int nSortId=0, IMatInfo * pIMatInfo=null);

  ushort *GetIndices(int *pIndicesCount);
  void DestroyIndices();

  void UpdateVidIndices(const ushort *pNewInds, int nInds);
  void UpdateSysIndices(const ushort *pNewInds, int nInds);

  void UpdateSysVertices(void * pNewVertices, int nNewVerticesCount);
  void UpdateVidVertices(void * pNewVertices, int nNewVerticesCount);
  bool CreateSysVertices(int nVerts, int VertFormat);
  void CreateVidVertices(int nVerts, int VertFormat);

  void SetRECustomData(float * pfCustomData, float fFogScale=0, float fAlpha=1);

  void SetChunk(  IShader * pShader, 
                          int nFirstVertId, int nVertCount, 
                          int nFirstIndexId, int nIndexCount, int nMatID = 0,
													bool bForceInitChunk = false);

  int									m_nClientTextureBindID;

  void SetShader( IShader * pShader, int nCustomTID = 0 );

  void   * GetSecVerticesPtr(int * pVerticesCount);

	//! /param Flags ????
	//! /return memory consumption of the object in ubytes
  int Size( int Flags );

  Vec3								m_vBoxMin, m_vBoxMax;							//!< used for hw occlusion test
  Vec3 *							m_pLoadedColors;									//!<

  void AllocateSystemBuffer( int nVertCount );
  void FreeSystemBuffer();
  bool Serialize(ref int nPos, ubyte * pSaveBuffer, bool bSaveToBuffer, 
    char * szFolderName, char * szFileName, ref double  dCIndexedMesh__LoadMaterial);
  void SerializeMemBuffer(ubyte * pSerialBuffer, ubyte * pData, int nSize, bool bSaveToBuffer);
	void DrawImmediately();
  bool LoadMaterial(int m, 
    const char *szFileName, const char *szFolderName, 
    ref list2!CMatInfo  lstMatTable, IRenderer * pRenderer,
    MAT_ENTITY * me, bool bFake);
  static int SetTexType(TextureMap3 *tm);
	bool UpdateTangBuffer(SPipTangents *pBasis);
	bool IsEmpty() { return !m_SecVertCount	|| !m_pSecVertBuffer || !m_SecIndices.Num(); }
	void RenderDebugLightPass(ref const Matrix44 mat, int nLightMask, float fAlpha);
	void Render(ref const SRendParams  rParams, CCObject * pObj, ref TArray!int  ShaderTemplates, int e_overlay_geometry, bool bNotCurArea, IMatInfo *pMaterial, bool bSupportDefaultShaderTemplates);
};


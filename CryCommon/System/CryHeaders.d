...
struct CryFace
{
	...
	//FIXME:
	//ref int operator [] (int i) {return (&v0)[i];}
	//FIXME:
...
{
	bool operator () (const ref CryFace faceLeft, const ref CryFace faceRight)const
	{
...
struct CryTexFace
{
	...
	this (int _t0,int _t1, int _t2)
	{
		t0(_t0);t1(_t1);t2(_t2);
	}
	//FIXME:
	//ref int operator[] (int i) {return (&t0)[i];}
	//FIXME:
...
struct TextureMap2
{
	char[32]	name;	// Name of file.

	ubyte	type;	  // Mapping type.
	ubyte	flags;	// Mapping type.
  ubyte Amount;
	ubyte[1] Reserved;
...
struct TextureMap3
{
  ...
  //FIXME:
  /*
  ref TextureMap3 operator = (ref const TextureMap2 tm)
  {
    strcpy(name, tm.name);
    type = tm.type;
    umirror = tm.umirror;
...
{
	bool operator () (ref const CryLink left, ref const CryLink right)
	{
...
struct BONE_PHYSICS
{
	...
	int		flags;
	float[3] min;float[3] max;
	float[3] spring_angle;
	float[3] spring_tension;
	float[3] damping;
	float[3][3] framemtx;
};
...
struct BONE_ENTITY
{
	...
	char[32]	prop;	// do not forget to update strncpy len // [Sergiy] why not use sizeof??? :)
...
//////////////////////////////////////////////////////////////////////
class MAT_ENTITY_DATA
{
	char[64]	name;	  //material/shader name
	...
	float[8] m_Temp; // For future modifications
...
struct MAT_ENTITY
{
	MAT_ENTITY_DATA data;
	int		*m_pMaterialChildren;
};

//////////////////////////////////////////////////////////////////////
struct MAT_ENTITY_COMP
{
	MAT_ENTITY_DATA data;
...
//////////////////////////////////////////////////////////////////////
void CopyMatEntity(ref MAT_ENTITY left, ref const MAT_ENTITY_COMP right)
{
	cast(MAT_ENTITY_DATA)left = cast(MAT_ENTITY_DATA)right;
	left.m_pMaterialChildren = cast(int*)right.m_iMaterialChildren;
}

//////////////////////////////////////////////////////////////////////
void CopyMatEntity(ref MAT_ENTITY_COMP left, ref const MAT_ENTITY right)
{
	cast(MAT_ENTITY_DATA)left = cast(MAT_ENTITY_DATA)right;
	left.m_iMaterialChildren = cast(int)right.m_pMaterialChildren;
}
...
//File Headers
//////////////////////////////////////////////////////////////////////
immutable FILE_SIGNATURE =	"CryTek";
...
enum GeomFileVersion =		0x0744;
enum AnimFileVersion =		0x0744;
...
alias CHUNK_HEADER_0744 CHUNK_HEADER;
...
alias TIMING_CHUNK_DESC_0918 TIMING_CHUNK_DESC;
//FIXME:
...
alias MTL_CHUNK_DESC_0744 MTL_CHUNK_DESC;
alias MTL_CHUNK_DESC_0746 MTL_CHUNK_DESC_EXPORTER;
...
struct LIGHT_CHUNK_DESC_0744
{
	...
	char[256]		szLightImage;	//spot light texture
};

alias LIGHT_CHUNK_DESC_0744 LIGHT_CHUNK_DESC;
//FIXME:
...
alias MESH_CHUNK_DESC_0744 MESH_CHUNK_DESC;
//FIXME:
...
//alias SCENEPROPS_CHUNK_DESC_0744 SCENEPROPS_CHUNK_DESC;
//FIXME:
...
enum MRM_CHUNK_DESC_VERSION = 0x0201;
...
enum DUMMY_CHUNK_DESC_VERSION = 0x0201;
...
alias VERTANIM_CHUNK_DESC_0744 VERTANIM_CHUNK_DESC;
enum VERTANIM_CHUNK_DESC_VERSION = 0x0744;
...
alias BONEANIM_CHUNK_DESC_0290 BONEANIM_CHUNK_DESC;
...
alias GEOMNAMELIST_CHUNK_DESC_0201 GEOMNAMELIST_CHUNK_DESC;
...
alias MTLLIST_CHUNK_DESC_0201 MTLLIST_CHUNK_DESC;
...
struct BaseKey1 {BaseKey bk; float		val; };
struct BaseKey3 {BaseKey bk; Vec3	val; };
struct BaseKeyQ {BaseKey bk; CryQuat	val; };

alias BaseKey1  CryLin1Key;
alias BaseKey3 CryLin3Key;
alias BaseKey QCryLinQKey;
struct CryTCB1Key { BaseKey1 bk;  BaseTCB btcb;}
struct CryTCB3Key { BaseKey3 bk; BaseTCB btcb; }
struct CryTCBQKey { BaseKeyQ bk; BaseTCB btcb; }
struct CryBez1Key { BaseKey1 bk; float		intan, outtan; }
struct CryBez3Key { BaseKey3 bk;  Vec3	intan, outtan; }
struct CryBezQKey { BaseKeyQ bk;}
struct CryBoneKey
{ 
	BaseKey bk;
	Vec3	abspos;
...
struct CONTROLLER_CHUNK_DESC_0826
{
  ...
  uint nControllerId; // unique generated in exporter id based on crc32 of bone name
};
...
alias CONTROLLER_BSPLINE_DATA_0826 CONTROLLER_BSPLINE_DATA;


enum CRY_RENDERMODE_SHADOW =   0;
enum CRY_RENDERMODE_TEXTURED = 1;
enum CRY_RENDERMODE_BUMP =     2;
enum CRY_RENDERMODE_REFRACT =  3;
...
alias NODE_CHUNK_DESC_0823 NODE_CHUNK_DESC;
...
alias HELPER_CHUNK_DESC_0744 HELPER_CHUNK_DESC; 
//FIXME:
...
{
	enum {VERSION = 0x0001};
	uint numBindings; // the number of bone-light binding substructures following this structure
};
...
{
	uint nLightChunkId;     // chunk id of the light that's bound to a bone
	uint nBoneId;           // parent bone id
...
{
	enum {VERSION = 0x0001};
	uint nChunkIdMesh;   // the chunk id of the mesh chunk (ChunkType_Mesh) for which this morph target is
	uint numMorphVertices;  // number of MORPHED vertices
};
...
{
	uint nVertexId; // vertex index in the original (mesh) array of vertices
...
struct SBoneInitPosMatrix
{
	...
	ref const(Vec3) getOrt (int nOrt)const {return *cast(const Vec3*)(mx[nOrt]);}
};
